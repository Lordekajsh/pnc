...
async def msg_buy_user(
    ...
    info = (
        f"<b>⏳ • SEU TEMPO PARA O REEMBOLSO: {time_max}. ({time} minutos)</b>"
        if exchange_on
        else ""
    )
    new_balance = cur.execute(
    ...
    base = f"""<b>🏦 | CARTÃO APROVADO MARCHA!!
    
   - NÃO FAÇO TROCAS NO PV!!
   - GARANTO APENAS LIVE!!
...
<b>🏆 Dados para aprovação:
@DADOS_NOMECPF</b>

<b>▪︎Nível:</b> {level_cc}
<b>▪︎Bandeira:</b> {vendor}
<b>▪︎País:</b> {country}
<b>▪︎Banco:</b> {bank}

<b>🏷️ Preço:</b> R$ {price}
<b>💸 Saldo atual:</b> R$ {new_balance}
...
def msg_group_client(
    mention, card, level, type_buy, price, gate, new_balance, vendor
) -> str:
    produto = f"""✅ <b>CC PASSADA NO CHECKER APROVADA!</b>

》 <b>Nível:</b> {level}
️》 <b>Bandeira:</b> {vendor}
✅ <b>Preço:R$ </b> {price}

🎭🤟 <b> {mention} obrigado pela preferência!</b>"""
    return produto


def msg_mix_group_cliente(mention, quantity, price, new_balance) -> str:
    produto = f"""📦 <b>COMPRA DE MIXS EFETUADA COM SUCESSO!</b>
